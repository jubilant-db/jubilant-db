...
bool ReadExact(int fd, std::byte* buffer, std::size_t length) {
  std::size_t offset = 0;
  while (offset < length) {
    const auto bytes = ::recv(fd, buffer + offset, length - offset, MSG_WAITALL);
    if (bytes <= 0) {
      return false;
    }
  ...
  while (running_.load() && connection->active.load()) {
    ...
    if (!RegisterTransaction(connection, request->id)) {
      ...
      for (const auto& operation : request->operations) {
        ...
        op.success = false;
        duplicate.operations.push_back(std::move(op));
      }
    ...
    if (!server_.SubmitTransaction(*request)) {
      ...
      for (const auto& operation : request->operations) {
        ...
        op.type = operation.type;
        op.key = operation.key;
        rejected.operations.push_back(std::move(op));
      }
...
bool NetworkServer::ReadFrame(int fd, std::string& payload) const {
  std::array<std::byte, 4> prefix{};
  if (!ReadExact(fd, prefix.data(), prefix.size())) {
    return false;
  }
  ...
  return ReadExact(fd, reinterpret_cast<std::byte*>(payload.data()), length);
}
...
void NetworkServer::CleanupConnection(const std::shared_ptr<Connection>& connection) {
  ...
  const auto iter = std::find(connections_.begin(), connections_.end(), connection);
...
bool NetworkServer::RegisterTransaction(const std::shared_ptr<Connection>& connection,
                                        std::uint64_t txn_id) {
  std::lock_guard guard(connections_mutex_);
  if (pending_results_.find(txn_id) != pending_results_.end()) {
    return false;
  }
...
std::optional<txn::TransactionRequest>
NetworkServer::DecodeRequest(const std::string& payload) const {
...
std::optional<txn::TransactionRequest>
NetworkServer::DecodeRequest(const nlohmann::json& json) const {
  if (!json.is_object()) {
    return std::nullopt;
  }
  ...
  const auto id = txn_it->get<std::uint64_t>();
...
std::optional<txn::Operation>
NetworkServer::DecodeOperation(const nlohmann::json& operation_json) const {
...
NetworkServer::DecodeRecord(const nlohmann::json& value_json) const {
...
nlohmann::json NetworkServer::EncodeResponse(const TransactionResult& result) const {
  nlohmann::json json;
  ...
  for (const auto& op_result : result.operations) {
    nlohmann::json op;
    op["type"] = OperationTypeToString(op_result.type);
    op["key"] = op_result.key;
    op["success"] = op_result.success;
...
NetworkServer::EncodeRecord(const storage::btree::Record& record) const {
  nlohmann::json value;
  bool supported = true;
...
std::optional<std::vector<std::byte>> NetworkServer::DecodeBytes(const std::string& encoded) {
  static constexpr std::array<int, 256> kReverse = [] {
    ...
    for (std::size_t i = 0; i < alphabet.size(); ++i) {
      table[static_cast<std::size_t>(alphabet[i])] = static_cast<int>(i);
    }
    return table;
  }();
  ...
  for (const auto ch : encoded) {
    ...
    const auto decoded = kReverse[static_cast<std::size_t>(static_cast<unsigned char>(ch))];
    if (decoded == -1) {
      return std::nullopt;
    }
