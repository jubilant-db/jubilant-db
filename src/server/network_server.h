...
private:
  ...
  bool ReadFrame(int fd, std::string& payload) const;
  bool WriteFrame(const std::shared_ptr<Connection>& connection, std::string_view payload) const;
  ...
  std::optional<txn::TransactionRequest> DecodeRequest(const std::string& payload) const;
  std::optional<txn::TransactionRequest> DecodeRequest(const nlohmann::json& json) const;
  std::optional<txn::Operation> DecodeOperation(const nlohmann::json& operation_json) const;
  std::optional<storage::btree::Record> DecodeRecord(const nlohmann::json& value_json) const;

  nlohmann::json EncodeResponse(const TransactionResult& result) const;
  std::optional<nlohmann::json> EncodeRecord(const storage::btree::Record& record) const;
